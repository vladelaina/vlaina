...
import type { RowMenuAction } from './row-header-drag-state'
import type { DragSession, PressSession } from './column-header-drag-types'
import { useRowHeaderDragViewState } from './row-header-drag-view'
import {
  acquireTableDragCursor,
  releaseTableDragCursor,
  suppressTableDragSelection,
} from './drag-cursor'
import { getDragOverRow } from '../dnd/calc-drag-over'

interface UseRowHeaderDragOptions {
  ctx: Ctx
  ...
  clearRowContent: (index: number) => void
...
export function useRowHeaderDrag({
  ctx,
  tableWrapperRef,
  contentWrapperRef,
  moveRow,
  ...
  clearRowContent,
  deleteRow,
}: UseRowHeaderDragOptions) {
  const menuId = `row-header-drag-menu-${menuIdSequence += 1}`
  ...
  const getTargetIndexFromPointer = (clientY: number) => {
    ...
    const result = getDragOverRow(content, clientY)
  ...
  const beginDrag = (session: PressSession) => {
    ...
    activeRowIndex.value = session.index
    acquireTableDragCursor('row-resize')
    syncControls()
  }
  ...
  const handleWindowPointerDown = (event: PointerEvent) => {
    ...
    if (
      ...
      target.closest(
        '[data-role="row-header-drag-control"], [data-role="row-header-drag-menu"]'
      )
    ) {
      return
    }
  ...
  const onMenuAction = (action: RowMenuAction) => {

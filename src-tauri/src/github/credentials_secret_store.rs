...
pub(crate) struct GitHubSecretBundle {
    pub access_token: String,
    pub app_session_token: Option<String>,
}

pub(crate) const SECRET_SERVICE_NAME: &str = "NekoTick";
pub(crate) const ACCESS_TOKEN_ACCOUNT: &str = "github-access-token";
...
pub(crate) fn load_secret_bundle() -> Result<Option<GitHubSecretBundle>, String> {
    with_secret_store_lock(|| {
        let access_token = match read_secret(ACCESS_TOKEN_ACCOUNT)? {
            Some(value) => value,
            None => return Ok(None),
        };

        let app_session_token = read_secret(SESSION_TOKEN_ACCOUNT)?;
        Ok(Some(GitHubSecretBundle {
            access_token,
            app_session_token,
        }))
    })
...
pub(crate) fn save_secret_bundle(bundle: &GitHubSecretBundle) -> Result<(), String> {
    with_secret_store_lock(|| {
        write_secret(ACCESS_TOKEN_ACCOUNT, &bundle.access_token)?;
        if let Some(session_token) = bundle.app_session_token.as_deref() {
            write_secret(SESSION_TOKEN_ACCOUNT, session_token)?;
        } else {
...
pub(crate) fn delete_secret_bundle() -> Result<(), String> {
    with_secret_store_lock(|| {
        delete_secret(ACCESS_TOKEN_ACCOUNT)?;
        delete_secret(SESSION_TOKEN_ACCOUNT)?;
        Ok(())
    })
}

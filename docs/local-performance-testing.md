...
```bash
PERF_PROFILE=heavy PERF_SCENARIO=chat PERF_CHAT_MODEL_ID=your-local-model-id PERF_CONCURRENCY=50 PERF_DURATION_SECONDS=300 pnpm perf:api:local
```

默认 `PERF_CHAT_MODEL_ID=perf-model-000001`。如果这个模型对应的本地渠道 secret 没配好，请求会停在转发前置检查；要测完整上游转发，请把该模型的渠道 secret 指向本地假上游或测试上游。流式请求可加 `PERF_CHAT_STREAM=1`。
...
chat            /v1/chat/completions，使用 seed 生成的本地 app session
...
mixed-chat      客户端缓存口径 public + chat + budget
```
...
PERF_CHAT_MODEL_ID         chat 场景使用的模型 ID，默认 perf-model-000001
PERF_CHAT_STREAM           chat 场景是否使用 stream，默认 0
```

...
export function ChatView({ mode = 'full' }: ChatViewProps) {
  ...
  const isMessagesLoaded = useUnifiedStore((state) => {
    ...
    const sessions = state.data.ai?.sessions || [];
  ...
  const switchMessageVersionRef = useRef(aiActions.switchMessageVersion);
  ...
  useEffect(() => {
    switchMessageVersionRef.current = aiActions.switchMessageVersion;
  }, []);

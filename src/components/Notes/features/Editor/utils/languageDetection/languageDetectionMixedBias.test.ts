...
import {
  ...
  type LanguageDetectionMixedBiasFixture,
} from './languageDetectionMixedBias.fixtures';

interface MixedBiasFailure {
  actual: string | null;
  sample: string;
}

interface MixedBiasResult {
  fixture: LanguageDetectionMixedBiasFixture;
  matched: number;
  total: number;
  accuracy: number;
  failures: MixedBiasFailure[];
}

function runFixture(fixture: LanguageDetectionMixedBiasFixture, index: number): MixedBiasResult {
  const samples = fc.sample(fixture.arbitrary, {
    numRuns: fixture.sampleCount,
    seed: LANGUAGE_DETECTION_MIXED_BIAS_SEED + index,
  });

  const failures: MixedBiasFailure[] = [];
  let matched = 0;
...
function createReport(results: readonly MixedBiasResult[]) {
  const total = results.reduce((sum, result) => sum + result.total, 0);
  const matched = results.reduce((sum, result) => sum + result.matched, 0);

  return [
    `language detection mixed-bias seed=${LANGUAGE_DETECTION_MIXED_BIAS_SEED}`,
    ...results.map((result) => {
...
describe('guessLanguage mixed-block bias', () => {
  const results = languageDetectionMixedBiasFixtures.map(runFixture);
  const total = results.reduce((sum, result) => sum + result.total, 0);
  const matched = results.reduce((sum, result) => sum + result.matched, 0);
  const overallAccuracy = matched / total;

  it('prefers the dominant wrapper language for mixed snippets', () => {
  ...
  it('prints the current mixed-bias report', () => {

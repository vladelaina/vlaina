...
const shellCommandLineCases: readonly string[] = [
  'npm run build',
...
const cases: readonly AdversarialCase[] = [
  ...
  {
    language: 'bash',
    sample: 'cd api\n\npnpm exec wrangler tail api-vlaina-com\n  --config ./wrangler.toml --format pretty',
  },
  { language: 'bash', sample: 'sudo apt update' },
  { language: 'bash', sample: 'COREPACK_ENABLE_DOWNLOAD_PROMPT=0 pnpm install' },
  { language: 'bash', sample: 'env NODE_ENV=production pnpm exec vite build' },
  { language: 'bash', sample: './scripts/deploy.sh --target production' },
  { language: 'bash', sample: 'chmod +x ./scripts/deploy.sh\n./scripts/deploy.sh' },
  { language: 'bash', sample: 'rsync -av ./dist/ user@example.com:/srv/app/' },
  { language: 'bash', sample: 'ssh deploy@example.com "systemctl restart api"' },
  ...shellCommandLineCases.map((sample) => ({ language: 'bash', sample })),

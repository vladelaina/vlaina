...
describe('slashFileCommands security helpers', () => {
  it('only treats image paths as insertable images', () => {
    expect(__testing__.isInsertableImagePath('/notesRoot/assets/cover.png')).toBe(true);
    expect(__testing__.isInsertableImagePath('/notesRoot/assets/secret.md')).toBe(false);
  });

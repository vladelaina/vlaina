...
const EMPTY_SLASH_EMOJI_PREVIEW_STATE: SlashEmojiPreviewState = {
  emoji: null,
  pos: null,
};
...
export const slashEmojiPreviewPlugin = $prose(() => new Plugin({
  ...
  state: {
    ...
    apply(tr, previous) {
      ...
      if (meta) {
        return meta as SlashEmojiPreviewState;
      }
      if (tr.docChanged) {
...
export function openSlashEmojiPicker(view: EditorView) {
  ...
  activeSlashEmojiPicker = new SlashEmojiPickerSession(view);

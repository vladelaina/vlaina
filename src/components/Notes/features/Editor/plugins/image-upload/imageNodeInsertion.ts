...
function replaceSelectionWithImageNode(
    ...
    return replaceVisibleBlockSelectionWithCursor(view, tr).replaceSelectionWith(imageNode);
}
...
export function canInsertImageNodeAtSelection(view: EditorView, selection?: Selection): boolean {
    ...
    try {
        const tr = replaceSelectionWithImageNode(view, imageNode, selection);
...
export function insertImageNodeAtSelection(view: EditorView, src: string, selection?: Selection): boolean {
    ...
    try {
        ...
        view.dispatch(replaceSelectionWithImageNode(view, imageNode, selection).scrollIntoView());
        return true;
    } catch (error) {
        return false;
    }

...
export function createCollapseTriangleSvgMarkup(size = themeIconTokens.sizeCollapseTriangle): string {
  ...
  return `<svg aria-hidden="true" focusable="false" width="${normalizedSize}" height="${normalizedSize}" viewBox="${COLLAPSE_TRIANGLE_VIEW_BOX}" fill="${themeStyleResetTokens.fillNone}" stroke="${themeStyleResetTokens.currentColor}" stroke-width="${themeIconTokens.strokeCollapseTriangle}" stroke-linecap="round" stroke-linejoin="round"><path d="${COLLAPSE_TRIANGLE_PATH}"/></svg>`;
}

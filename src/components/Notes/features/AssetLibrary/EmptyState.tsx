...
export function EmptyState({ onUploadClick, compact }: EmptyStateProps) {
  return (
    <div className={cn("flex flex-col items-center justify-center px-4", compact ? "py-6" : "py-12")}>
      <div className={cn(
        "rounded-full bg-[var(--vlaina-bg-tertiary)] flex items-center justify-center mb-3",
        compact ? "w-10 h-10" : "w-16 h-16 mb-4"
      )}>
        <Icon name="file.imagePlus" className={cn("text-[var(--vlaina-text-tertiary)]", compact ? "w-5 h-5" : "w-8 h-8")} />
      </div>

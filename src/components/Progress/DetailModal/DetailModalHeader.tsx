...
export function DetailModalHeader({
  ...
  return (
    <div className="relative z-20 flex justify-between items-center p-6 px-8 h-20">
      ...
      {/* Right: Action Area (Adaptive) */}
      <div className="relative h-10 flex items-center justify-end z-50">
        {isArchived ? (
          /* Scenario A: Archived Item -> Direct Delete */
          <button
            onClick={onDelete}
            className={`
              group relative size-10 flex items-center justify-center rounded-full
          ...
          >
            <Trash 
              weight="duotone" 
              className="size-5 text-zinc-400 group-hover:text-red-500 transition-colors duration-300" 
            />
          </button>
        ) : (
          /* Scenario B: Active Item -> Full Menu */
          <motion.div
            layout
            className={`

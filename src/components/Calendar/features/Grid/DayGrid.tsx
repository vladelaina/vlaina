...
export function DayGrid() {
  const { selectedDate, addEvent, dayCount } = useCalendarStore();
  ...
  const handleMouseUp = useCallback(() => {
    ...
    setDragStart(null);
    setDragEnd(null);
  }, [isDragging, dragStart, dragEnd, days, addEvent]);
        ...
        <div className="flex min-h-full relative" ref={containerRef} onMouseDown={handleMouseDown}>
          ...
          <div className="flex-1 relative cursor-crosshair">
            ...
            <div className={`absolute inset-0 grid z-0 pointer-events-none`} style={{ gridTemplateColumns: `repeat(${dayCount}, 1fr)` }}>
              {days.map((day, i) => (
                <div key={i} className={`border-r border-zinc-100 dark:border-zinc-800/50 h-full ${isSameDay(day, now) ? 'bg-red-50/10 dark:bg-red-900/5' : ''}`} />
              ))}
            </div>
            ...
            {isDragging && dragStart && (
              <div
                style={getGhostStyle()!}
                className="absolute z-30 bg-blue-500/20 border-2 border-blue-500 rounded-md pointer-events-none flex items-center justify-center"
              >

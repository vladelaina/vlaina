...
export const UserIdentityCard: React.FC<UserIdentityCardProps> = ({ onLogout, onSwitchAccount }) => {
  ...
  const shouldShowMembershipBadge = isConnected && !isMembershipPending;
  ...
  const membershipPillClassName = isConnected
    ...
    : 'bg-white text-neutral-500 dark:bg-zinc-900 dark:text-neutral-400';
      ...
      <div className="relative shrink-0 group/avatar">
        ...
        {shouldShowMembershipBadge ? (
          <span
            ...
            className={cn(
              'absolute -bottom-1.5 -right-2 z-10 inline-flex cursor-pointer select-none items-center rounded-[10px] px-2 py-1 text-[9px] font-bold uppercase tracking-[0.14em] leading-none shadow-[0_8px_18px_rgba(15,23,42,0.08)]',
              membershipPillClassName
            )}
          >
            {membershipName || (membershipTier === 'free' ? 'Free' : '')}
          </span>
        ) : null}
      </div>

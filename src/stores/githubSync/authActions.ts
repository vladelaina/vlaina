...
import type { GithubSyncState, GithubSyncActions, GithubSyncStatusType } from './state';
import { GITHUB_USER_PERSIST_KEY } from './state';
...
export function createCheckStatus(set: Set, get: Get): () => Promise<void> {
  return async () => {
    ...
    if (hasBackendCommands()) {
      try {
        const status = await githubSyncCommands.getGithubSyncStatus();
        if (status) {
          set({
...
export function createConnect(set: Set, get: Get): () => Promise<boolean> {
  return async () => {
    ...
    if (hasBackendCommands()) {
      try {
        const result = await githubSyncCommands.githubAuth();

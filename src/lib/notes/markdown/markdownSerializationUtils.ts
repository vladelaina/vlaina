...
function normalizeRedundantMarkdownEscapesInSegment(segment: string): string {
  let output = '';
...
function isRedundantMarkdownEscape(segment: string, slashIndex: number, escaped: string): boolean {
  if (escaped === '_') {
...
function isRedundantUnderscoreEscape(segment: string, slashIndex: number): boolean {
...
function isRedundantOpeningBracketEscape(segment: string, slashIndex: number): boolean {
  return isNonWhitespace(segment[slashIndex - 1]);
}

function isEscapedAbbreviationDefinitionMarker(segment: string, slashIndex: number): boolean {
  const { line, offset } = getLineAtIndex(segment, slashIndex);
  return /^[ \t]*$/.test(line.slice(0, offset))
    && /^\\\*\\?\[[^\]\n]+]:(?=\s|$)/.test(line.slice(offset));
}

function isEscapedAbbreviationDefinitionBracket(segment: string, slashIndex: number): boolean {
  const { line, offset } = getLineAtIndex(segment, slashIndex);
  return /^[ \t]*\\\*$/.test(line.slice(0, offset))
    && /^\\\[[^\]\n]+]:(?=\s|$)/.test(line.slice(offset));
}

function isRedundantHeadingMarkerEscape(segment: string, slashIndex: number): boolean {
...
function isSingleMarkerInCurrentTextToken(
  segment: string,
  slashIndex: number,
  marker: string
): boolean {
...
function countMarkerInSegment(segment: string, marker: string): number {
  return countMarkerInRange(segment, marker, 0, segment.length);
}

function countMarkerInRange(
  segment: string,
  marker: string,
  start: number,
  end: number
...
function getNonWhitespaceTokenBounds(segment: string, index: number): { start: number; end: number } {
  let start = index;
...
function getLineAtIndex(segment: string, index: number): { line: string; offset: number } {
  const lineStart = segment.lastIndexOf('\n', index - 1) + 1;
...
function isNonWhitespace(value: string | undefined): value is string {
...
function normalizeGenericHtmlBlockClosingSpacingSegment(text: string): string {
...
function normalizeRenderedHtmlBoundaryHelperCommentSegment(
  text: string,
  startIndex: number,

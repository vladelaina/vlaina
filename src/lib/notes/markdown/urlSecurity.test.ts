...
describe('urlSecurity', () => {
  ...
  it('blocks local-network media URLs without a browser window', () => {
    ...
    expect(isLocalNetworkHttpUrl('http://2130706433/image.png')).toBe(true);
    expect(isLocalNetworkHttpUrl('http://0x7f000001/image.png')).toBe(true);
    expect(isLocalNetworkHttpUrl('http://0177.0.0.1/image.png')).toBe(true);
    ...
    expect(sanitizeNoteMediaSrc('http://2130706433/image.png')).toBeNull();
  ...
  it('allows public media URLs without a browser window', () => {
    ...
    expect(sanitizeNoteMediaSrc('https://fc.example.com/image.png')).toBe('https://fc.example.com/image.png');
  ...
  it('does not classify unsafe remote strings as public media URLs', () => {
    ...
    expect(normalizePublicRemoteMediaUrl(String.raw`//example.com\image.png`)).toBeNull();
    ...
    expect(sanitizeNoteMediaSrc(String.raw`//example.com\image.png`)).toBeNull();
    expect(sanitizeNoteMediaSrc('https:example.com/image.png')).toBeNull();

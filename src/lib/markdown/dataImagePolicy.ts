...
const MAX_SAFE_RASTER_DATA_IMAGE_SRC_CHARS = MAX_INLINE_IMAGE_BASE64_CHARS + 128;
...
export function normalizeSafeRasterDataImageSrc(src: string | null | undefined): string | null {
  ...
  if (src.length > MAX_SAFE_RASTER_DATA_IMAGE_SRC_CHARS) {
    return null;
  }

  const trimmed = src.trim();
  if (trimmed.length > MAX_SAFE_RASTER_DATA_IMAGE_SRC_CHARS) {
    return null;
  }

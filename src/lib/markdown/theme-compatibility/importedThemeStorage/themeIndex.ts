...
import { hasInvalidImportedThemeFileSize, MAX_IMPORTED_THEME_INDEX_BYTES } from './constants';
...
const themeIndexUtf8Encoder = new TextEncoder();
...
export async function readThemeIndex(): Promise<ImportedMarkdownThemeMetadata[]> {
  ...
  try {
    ...
    if (
      ...
      hasInvalidImportedThemeFileSize(info, MAX_IMPORTED_THEME_INDEX_BYTES)
    ) {
      return [];
    }
    const content = await storage.readFile(indexPath, MAX_IMPORTED_THEME_INDEX_BYTES);
    if (themeIndexUtf8Encoder.encode(content).length > MAX_IMPORTED_THEME_INDEX_BYTES) {
      return [];
    }

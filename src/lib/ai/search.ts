...
export async function performWebSearch(query: string): Promise<SearchResult[]> {
    ...
    if (isTauri()) {
        try {
            ...
            // Fallback? No, if Tauri fails, fetch will definitely fail due to CORS.
            return [];
        }
    } else {
        // Dev environment / Browser fallback (Will likely fail due to CORS unless proxied)
        console.warn('[Search] Running in browser mode. CORS might block this request.');
        // We can use a public CORS proxy for dev demo purposes, but for now let's stick to the direct call
        // and let it fail with a clear message if blocked.
        return fallbackBrowserSearch(query);
    }
}

async function fallbackBrowserSearch(query: string): Promise<SearchResult[]> {
    // This is for dev environment only. In production, always use Tauri command.
    // Using a public CORS proxy (e.g. corsproxy.io) is risky/unstable but works for quick dev test.
